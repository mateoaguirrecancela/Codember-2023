...
int main(){
    ...
    for (int i = 0; i < tamano; i++){
        ...
        u.mail=strdup(aux[2]);
        u.age=strdup(aux[3]);
        u.location=strdup(aux[4]);

        if((strlen(u.id) <= 0 || !alfaNumerica(u.id)) || (strlen(u.name) <= 0 || !alfaNumerica(u.name)) || (strlen(u.mail) <= 0 || !correo(u.mail)) || (!numero(u.age)) || (!letra(u.location))){
            usuariosInvalidos[invalidos]=u;
            invalidos++;
        }
    }

    char solucion[invalidos + 1];
    solucion[0] = '\0';
    
    for (int i = 0; i < invalidos; i++){
        if(strlen(usuariosInvalidos[i].name) > 0){
            strncat(solucion,usuariosInvalidos[i].name, 1);
        }
    ...
    for (int i = 0; i < tamano; i++) {
        free(usuariosInvalidos[i].id);
        free(usuariosInvalidos[i].name);
        free(usuariosInvalidos[i].mail);
        free(usuariosInvalidos[i].age);
        free(usuariosInvalidos[i].location);
    }
